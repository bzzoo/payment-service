...
import com.app.paymentservice.payment.adapter.out.persistent.exception.PaymentValidationException
        ...
        return databaseClient.sql(SELECT_PAYMENT_TOTAL_AMOUNT_QUERY)
            ...
            .handle { row, sink ->
                if ((row["total_amount"] as BigInteger).toLong() == amount) {
                    ...
                    sink.error(PaymentValidationException(message = "결제 (orderID: $orderId) 에서 금액 (amount: ${amount}이 올바르지 않습니다."))
                }
            }
    }
