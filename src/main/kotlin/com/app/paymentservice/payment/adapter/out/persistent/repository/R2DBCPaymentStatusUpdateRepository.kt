...
import com.app.paymentservice.payment.adapter.out.persistent.exception.PaymentAlreadyProcessedException
...
class R2DBCPaymentStatusUpdateRepository(
    ...
    override fun updatePaymentStatus(command: PaymentStatusUpdateCommand): Mono<Boolean> {
        return when (command.status) {
            ...
            else -> error("결제 상태 (status: ${command.status}) 는 올바르지 않은 결제 입니다.")
        }
    }
    ...
    private fun updatePaymentEventExtraDetails(command: PaymentStatusUpdateCommand): Mono<Long> {
        return databaseClient.sql(UPDATE_PAYMENT_EVENT_EXTRADEATILS_QUERY)
    ...
    companion object {
        ...
        val UPDATE_PAYMENT_EVENT_EXTRADEATILS_QUERY = """
            UPDATE payemnt_events
